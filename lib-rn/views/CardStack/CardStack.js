...
class CardStack extends React.Component {
  ...
  constructor(props) {
    ...
    this._hasSplitPaneComponent = this._hasSplitPaneComponent.bind(this);
  }
  ...
  _renderHeader(scene, headerMode) {
    ...
    return (
      // $FlowFixMeRN0.51.1
      <this.props.headerComponent {...this.props} accessibilityOption={accessibilityOption} openDrawer={this.props.openDrawer} onNavigateBack={this.props.handleBackAction} scene={scene} mode={headerMode} transitionAnimation={transitionAnimation} isFlipTransition={this.props.isFlipTransition} />
    );
  }
  ...
  render() {
    ...
    return <Animated.View style={[containerStyle, flipAnimationStyle]}>
        <View style={styles.scenes}>
          ...
          const shouldHideFlipFromScene = !isHideTopScene && !isTopScene && isFlipTransition;
  ...
  _renderInnerScene(SceneComponent, scene) {
    ...
    return <View style={{ flex: 1, backgroundColor: theme.white }} testID={`Screen_${scene.route.routeName}_${isActiveRoute ? 'IsActive' : 'IsNotActive'}`}>
        <View style={{
        height: paddingTop,
        backgroundColor: theme.primaryBlue
      }} />
        <View style={{ flexDirection: 'row', flex: 1 }}>
          {hasSplitPaneComponent && SplitPaneComponent && <View style={{
          width: 300,
          borderRightWidth: 1,
          ...
          zIndex: 1
                ...
                <CardSceneView key={`SPLIT_PANE${route.key}`} routeProps={scene.route} component={SplitPaneComponent} scene={scene} handleNavigate={this.props.handleNavigate} handleBack={this.props.handleBackAction} trackingActions={this.props.trackingActions} hasModal={this.props.hasModal} isLeftSplitPaneComponent />
              </View>}
          <View style={{ flex: 1 }}>
            <CardSceneView {...route} key={scene.key} routeKey={route.key} routeProps={scene.route} component={SceneComponent} scene={scene} handleNavigate={this.props.handleNavigate} handleBack={this.props.handleBackAction} trackingActions={this.props.trackingActions} hasModal={this.props.hasModal} />
          </View>
        </View>
      </View>;
  }
  ...
  _renderCard = (scene, { isFlipTransition, shouldHide }) => {
    ...
    return <Card {...this.props} key={`card_${scene.key}`} style={[this.props.cardStyle, individualCardAnimation]} scene={scene}>
