  ...
  _startTransition(nextProps, nextScenes, indexHasChanged) {
    ...
    this.setState(nextState, async () => {
      ...
      if (this.props.isFlipTransition) {
        const { flipFromAnimation, flipToAnimation } = getFlipAnimations(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec);
        ...
        const animations = getRegularAnimation(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec, timing);
...
function getRegularAnimation(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec, timing) {
  return indexHasChanged && positionHasChanged ? [timing(progress, {
    ...transitionSpec,
    toValue: 1
  }), timing(position, {
    ...transitionSpec,
...
function getFlipAnimations(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec) {
  const flipFromAnimation = indexHasChanged && positionHasChanged ? [Animated.timing(progress, {
    ...transitionSpec,
    toValue: 0.5
  ...
  const flipToAnimation = indexHasChanged && positionHasChanged ? [Animated.timing(progress, {
    ...transitionSpec,
    toValue: 1
  }), Animated.timing(position, {
    ...transitionSpec,
