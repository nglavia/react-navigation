  ...
  _createClass(Transitioner, [{
    ...
    value: function _startTransition(nextProps, nextScenes, indexHasChanged) {
      ...
      this.setState(nextState, _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
        var result, _getFlipAnimations, flipFromAnimation, flipToAnimation, animations;
        ...
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              ...
              case 5:
                if (_this2.props.isFlipTransition) {
                  _getFlipAnimations = getFlipAnimations(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec), flipFromAnimation = _getFlipAnimations.flipFromAnimation, flipToAnimation = _getFlipAnimations.flipToAnimation;

                  _this2.props.onFlipStart();
                  _reactNative.Animated.parallel(flipFromAnimation).start(function (callback) {
                    ...
                    // point of the flip
                    _this2.props.onFlipFromComplete();
                    _reactNative.Animated.parallel(flipToAnimation).start(function (callback) {
                      _this2.props.onFlipToComplete();
                      _this2._onTransitionEnd();
                    });
                  });
                } else {
                  // if swiped back, indexHasChanged == true && positionHasChanged == false
                  animations = getRegularAnimation(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec, timing);
...
function getRegularAnimation(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec, timing) {
  return indexHasChanged && positionHasChanged ? [timing(progress, _extends({}, transitionSpec, {
    toValue: 1
...
function getFlipAnimations(indexHasChanged, positionHasChanged, progress, position, nextProps, transitionSpec) {
  var flipFromAnimation = indexHasChanged && positionHasChanged ? [_reactNative.Animated.timing(progress, _extends({}, transitionSpec, {
    toValue: 0.5
  })), _reactNative.Animated.timing(position, _extends({}, transitionSpec, {
    toValue: nextProps.navigation.state.index - 0.5
  }))] : [];
  var flipToAnimation = indexHasChanged && positionHasChanged ? [_reactNative.Animated.timing(progress, _extends({}, transitionSpec, {
    toValue: 1
  })), _reactNative.Animated.timing(position, _extends({}, transitionSpec, {
    toValue: nextProps.navigation.state.index
  }))] : [];
  ...
  return {
    flipFromAnimation: flipFromAnimation,
    flipToAnimation: flipToAnimation
  };
}
